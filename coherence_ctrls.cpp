...
uint64_t MESIBottomCC::processEviction(Address wbLineAddr, Address pc, uint32_t lineId, bool lowerLevelWriteback, uint64_t cycle, uint32_t srcId) {
    ...
    switch (*state) {
        ...
        case E:
            {
                MemReq req = {wbLineAddr, pc, PUTS, selfId, state, cycle, &ccLock, *state, srcId, 0 /*no flags*/};
                respCycle = parents[getParentId(wbLineAddr)]->access(req);
            }
            break;
        case M:
            {
                MemReq req = {wbLineAddr, pc, PUTX, selfId, state, cycle, &ccLock, *state, srcId, 0 /*no flags*/};
                respCycle = parents[getParentId(wbLineAddr)]->access(req);
            }
            break;
...
uint64_t MESIBottomCC::processAccess(Address lineAddr, Address pc, uint32_t lineId, AccessType type, uint64_t cycle, uint32_t srcId, uint32_t flags) {
    uint64_t respCycle = cycle;
    MESIState* state = &array[lineId];
    switch (type) {
        ...
        case GETS:
            if (*state == I) {
                ...
                MemReq req = {lineAddr, pc, GETS, selfId, state, cycle, &ccLock, *state, srcId, flags};
        ...
        case GETX:
            if (*state == I || *state == S) {
                ...
                uint32_t parentId = getParentId(lineAddr);
                MemReq req = {lineAddr, pc, GETX, selfId, state, cycle, &ccLock, *state, srcId, flags};
                uint32_t nextLevelLat = parents[parentId]->access(req) - cycle;
                uint32_t netLat = parentRTTs[parentId];
                profGETNextLevelLat.inc(nextLevelLat);
                profGETNetLat.inc(netLat);
...
uint64_t MESIBottomCC::processNonInclusiveWriteback(Address lineAddr, Address pc, AccessType type, uint64_t cycle, MESIState* state, uint32_t srcId, uint32_t flags) {
    ...
    MemReq req = {lineAddr, pc, type, selfId, state, cycle, &ccLock, *state, srcId, flags | MemReq::NONINCLWB};
