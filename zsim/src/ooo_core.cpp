...
inline void OOOCore::bbl(Address bblAddr, BblInfo* bblInfo) {
            ...
            case UOP_LOAD:
                {
                    ...
                    if (addr != ((Address)-1L)) {
                        reqSatisfiedCycle = l1d->load(addr, pc, dispatchCycle) + L1D_LAT;
            ...
            case UOP_STORE:
                {
                    ...
                    uint64_t reqSatisfiedCycle = l1d->store(addr, pc, dispatchCycle) + L1D_LAT;
                    cRec.record(curCycle, dispatchCycle, reqSatisfiedCycle);
    ...
    if (branchPc && !branchPred.predict(branchPc, branchTaken)) {
        ...
        for (uint32_t i = 0; i < 5*64/lineSize; i++) {
            uint64_t fetchLat = l1i->load(wrongPathAddr + lineSize*i, (Address)addr_hash(wrongPathAddr + lineSize*i), curCycle) - curCycle;
    ...
    for (Address fetchAddr = bblAddr; fetchAddr < endAddr; fetchAddr += lineSize) {
        ...
        uint64_t fetchLat = l1i->load(fetchAddr, (Address)addr_hash(fetchAddr), curCycle) - curCycle;
        cRec.record(curCycle, curCycle, curCycle + fetchLat);
        fetchCycle += fetchLat;
    }
