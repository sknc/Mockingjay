        ...
        case PUTX:
            {
                ...
                MemReq req = {acc.lineAddr, acc.pc, acc.type, acc.childId, &it->second, acc.reqCycle, nullptr, it->second, acc.childId};
        ...
        case GETX:
            {
                ...
                if (it != cStore.end()) {
                    if (!((it->second == S) && (acc.type == GETX))) { //we have the line, and it's not an upgrade miss, we can't replay this access directly
                        if (playAllGets) { //issue a PUT
                            MemReq req = {acc.lineAddr, acc.pc, (it->second == M)? PUTX : PUTS, acc.childId, &it->second, acc.reqCycle, nullptr, it->second, acc.childId};
                            parent->access(req);
                            assert(it->second == I);
                        } else {
                            return; //skip
                ...
                MemReq req = {acc.lineAddr, acc.pc, acc.type, acc.childId, &state, acc.reqCycle, nullptr, state, acc.childId};
