...
class MESIBottomCC : public GlobAlloc {
    ...
    public:
        ...
        uint64_t processEviction(Address wbLineAddr, Address pc, uint32_t lineId, bool lowerLevelWriteback, uint64_t cycle, uint32_t srcId);

        uint64_t processAccess(Address lineAddr, Address pc, uint32_t lineId, AccessType type, uint64_t cycle, uint32_t srcId, uint32_t flags);
...
class MESICC : public CC {
    ...
    public:
        ...
        uint64_t processEviction(const MemReq& triggerReq, Address wbLineAddr, int32_t lineId, uint64_t startCycle) {
            ...
            evCycle = bcc->processEviction(wbLineAddr, triggerReq.pc, lineId, lowerLevelWriteback, evCycle, triggerReq.srcId); //2. if needed, write back line to upper level
            return evCycle;
        }
        ...
        uint64_t processAccess(const MemReq& req, int32_t lineId, uint64_t startCycle, uint64_t* getDoneCycle = nullptr) {
            ...
            if (lineId == -1 || (((req.type == PUTS) || (req.type == PUTX)) && !bcc->isValid(lineId))) { //can only be a non-inclusive wback
                ...
                assert((req.type == PUTS) || (req.type == PUTX));
                respCycle = bcc->processNonInclusiveWriteback(req.lineAddr, req.pc, req.type, startCycle, req.state, req.srcId, req.flags);
            } else {
                ...
                respCycle = bcc->processAccess(req.lineAddr, req.pc, lineId, req.type, startCycle, req.srcId, flags);
...
class MESITerminalCC : public CC {
    ...
    public:
        ...
        uint64_t processEviction(const MemReq& triggerReq, Address wbLineAddr, int32_t lineId, uint64_t startCycle) {
            bool lowerLevelWriteback = false;
            uint64_t endCycle = bcc->processEviction(wbLineAddr, triggerReq.pc, lineId, lowerLevelWriteback, startCycle, triggerReq.srcId); //2. if needed, write back line to upper level
        ...
        uint64_t processAccess(const MemReq& req, int32_t lineId, uint64_t startCycle,  uint64_t* getDoneCycle = nullptr) {
            ...
            assert(!getDoneCycle);
            //if needed, fetch line or upgrade miss from upper level
            uint64_t respCycle = bcc->processAccess(req.lineAddr, req.pc, lineId, req.type, startCycle, req.srcId, req.flags);
            //at this point, the line is in a good state w.r.t. upper levels
            return respCycle;
        }
